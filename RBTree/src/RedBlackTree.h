...
#define LEAF NULL
	...
	struct Node
	{
		...
		inline void Print()
		{
			...
			{
				cout << m_key << "[" << color << ", " << m_pParent->m_key << "]";
			}
			else
			{
				cout << m_key << "[" << color << ", ROOT]";
			}
		}
	};
	...
	protected:
		...
		// 2. search
		bool searchHelper(Node<T> *n, T value);

		// 3. deletion
		Node<T>* findNodeToRemove(Node<T> *n, T value);
		Node<T>* finMinInRight(Node<T> *n);
		Node<T>* finMaxInLeft(Node<T> *n);
		...
		void levelorderHelper(Node<T> *n);

		// General helper API
		inline bool isLeftChild(Node<T> *n);
		inline bool isRightChild(Node<T> *n);

		inline Node<T>* parent(Node<T> *n);
		inline Node<T>* grandParent(Node<T> *n);
		inline Node<T>* sibling(Node<T> *n);
		inline Node<T>* uncle(Node<T> *n);

		inline bool isRed(Node<T> *n);
		inline bool isBlack(Node<T> *n);
	...
	template<typename T>
	bool RBTree<T>::searchHelper(Node<T> *n, T value)
	{
		if (n && n != LEAF)
		{
			if (value < n->m_key)
	...
	template<typename T>
	Node<T>* RBTree<T>::findNodeToRemove(Node<T> *n, T value)
	{
		if (!n || n == LEAF)
		{
			return NULL;
		...
		{
			findNodeToRemove(n->m_pLeft, value);
		}
		else if (value > n->m_key)
		{
			findNodeToRemove(n->m_pRight, value);
		}
		else
		{// Match found
			return n;
	...
	template<typename T>
	Node<T>* RBTree<T>::finMinInRight(Node<T> *n)
	{
		assert(n != NULL);
	...
	template<typename T>
	Node<T>* RBTree<T>::finMaxInLeft(Node<T> *n)
	{
		assert(n != NULL);
	...
	template<typename T>
	void RBTree<T>::inorderHelper(Node<T> *n)
	{
		if (n && n != LEAF)
		{
	...
	template<typename T>
	void RBTree<T>::preorderHelper(Node<T> *n)
	{
		if (n && n != LEAF)
		{
	...
	template<typename T>
	void RBTree<T>::postorderHelper(Node<T> *n)
	{
		if (n && n != LEAF)
		{
			n->Print();
	...
	template<typename T>
	void RBTree<T>::levelorderHelper(Node<T> *n)
	{
		if (n && n != LEAF)
		{
			using std::queue;
	...
	template<typename T>
	bool RBTree<T>::isLeftChild(Node<T> *n)
	{
		Node<T> *p = parent(n);
		if (p && p->m_pLeft == n)
		{
	...
	template<typename T>
	bool RBTree<T>::isRightChild(Node<T> *n)
	{
		Node<T> *p = parent(n);
		if (p && p->m_pRight == n)
		{
	...
	template<typename T>
	Node<T>* RBTree<T>::parent(Node<T> *n)
	{
		if (n)
		{
			return n->m_pParent;
	...
	template<typename T>
	bool RBTree<T>::isBlack(Node<T> *n)
	{
		if (n && n != LEAF)
		{
			return n->isBlack();
	...
	{
		Node<T>* n = new Node<T>(value, color);
		return n;
	}
